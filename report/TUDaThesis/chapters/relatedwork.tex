...
\begin{itemize}
    ...
    \item \textbf{Signal-aware reconstruction objectives} (Section~\ref{sec:rw-spectral})
...
as second-order damped harmonic oscillators \cite{zhao2025phasenet}.
...
captured signal; to address this gap, recent work based on the EMPD dataset
introduces event-based neuromorphic sensing, which offers microsecond-level
temporal resolution \cite{feng2026empd}.

The modalities relevant to this thesis extend beyond visible-spectrum rPPG.
Thermal infrared (TIR) imaging captures the long-wave infrared radiation
($8$--$14\,\mu m$) emitted by the skin and is therefore invariant to visible
illumination; it enables thermal respiration monitoring at the nostrils,
autonomic-stress assessment through nasal temperature drops, and the
quantification of sweat-gland activation \cite{cho2019physiological}. Because
these signatures are driven by the same autonomic processes that modulate the
cardiovascular signal, TIR constitutes a physiologically redundant yet
spectrally disjoint companion to RGB. A third ingredient is 3D facial geometry:
point-based tracking frameworks register spectral textures onto dynamic 3D
facial meshes and use standardized landmark sets---such as the 49-point 2D,
83-point 3D, and 28-point thermal landmarks of the BP4D+ corpus
\cite{zhang2014bp4d}---to define and track regions of interest across modalities
\cite{hassanpour2025contactless}. This geometric anchoring compensates for head
motion and perspective distortion that would otherwise mask the subtle chromatic
fluctuations exploited by rPPG \cite{cheng2024contactless}.

Each of these strands---algorithmic modeling, additional modalities, and higher
temporal resolution---assumes that the underlying sensor is functioning. None of
them can compensate for a sensor that has failed completely. These limitations
correspond exactly to the scenario investigated in this thesis and highlight the
need for \emph{cross-modal} redundancy rather than sensor-level redundancy
alone.
...
foundation for downstream health monitoring \cite{thukral2026wavelet}.

In a complementary line of work, rPPG-MAE adapts these self-supervised paradigms
...
contact-based signals. None of them addresses
...
Taken together, the reviewed literature leaves a distinct gap in understanding
cross-modal predictive limits. This thesis fills this gap by shifting the focus
from
