...
as PhysNet and PhysFormer demonstrate that vital signs can be extracted directly
...
as second-order damped harmonic oscillators \cite{zhao2024phasenet}. Beyond
algorithmic advances, the hardware used for physiological sensing has also
...
Two observations from this line of work motivate the thesis. First, existing
models are predominantly designed to estimate a physiological quantity from a
single, fully available video stream and provide no mechanism to recover a
signal when the corresponding sensor stream is entirely unavailable. Second,
recent efforts to improve robustness operate either at the algorithmic level
(physics-informed attention) or at the hardware level (event-based sensing), but
...
Critically, VideoMAE masks entire \emph{spatiotemporal tubes} that span the
...
relevant to the temporal masking investigated in Sub-RQ~2 of this thesis (the
research questions are defined in the introduction). VideoMAE V2 later scales
this framework and introduces a dual-masking strategy that masks both encoder
and decoder inputs, substantially reducing the computational overhead of
billion-parameter models \cite{wang2023videomaev2}. For multi-sensor
environments, MV2MAE extends the MAE paradigm to synchronized multi-view videos,
...
inference time \cite{bachmann2022multimae}. ImageBind provides a complementary
framework for learning joint embeddings across six disparate
...
\cite{girdhar2023imagebind}. Moving beyond symmetric alignment, xMAE reframes
cross-modal learning as a problem of \emph{asymmetric temporal observability}:
...
These frameworks establish that shared embedding spaces can align
physiologically related modalities and that a model can tolerate the absence of
...
characteristics jointly \cite{yu2026maeconformer}. To model the hierarchical
rhythms of biosignals explicitly, the MMR (Wavelet-Driven Masked Multiscale
...
\begin{tabular}{@{}p{2.4cm} p{2.5cm} p{3.3cm} p{2.2cm} p{2.4cm}@{}}
\toprule
Method & Modalities & Reconstruction objective & Within/Cross-modal & Sensor-failure focus \\
\midrule
PhysNet, PhysFormer \cite{savic2024physunet}
...
EMPD \cite{feng2026empd}
    & Event camera & event-driven rPPG estimation & Within (hardware) & No \\
VideoMAE \cite{tong2022videomae}
    & Video & tube-masked reconstruction (90--95\,\%) & Within & No \\
VideoMAE V2 \cite{wang2023videomaev2}
    & Video & dual-masked reconstruction & Within & No \\
MV2MAE \cite{shah2023mv2mae}
    & Multi-view video & cross-view reconstruction & Cross-view & Partial (viewpoint) \\
...
ImageBind \cite{girdhar2023imagebind}
    & Six modalities (incl.\ thermal, IMU) & joint embedding alignment & Cross-modal & Partial (no generation) \\
...
stream, be it video (VideoMAE, VideoMAE V2, MV2MAE) or physiological signal
