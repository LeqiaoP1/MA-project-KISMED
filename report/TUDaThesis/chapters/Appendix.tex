\chapter{Appendix - Christoph Hoog Antink's \LaTeX~Smartassery}
There should always be text between a section and a subsection.
\section{Various Points}
\begin{itemize}
    \item If the caption says ``Figure'', use ``see Figure'' when referencing. If it says ``Fig.'', use ``see Fig.'' in the text, etc.
    \item Use proper quotation marks, i.e. ``this is correct'', "this is incorrect".
    \item A variable should be set in italics, an identifier in Roman type. These subscripts are correct: $X_i$ with $i \in \Re$, $X_\mathrm{max} = 5$. This is wrong: $X_\mathrm{i}$ with $\mathrm{i} \in \Re$, $X_{max} = 5$. Operators should also be in Roman type, e.g. $\mathrm{arg max}\left(x\right)$, $\mathrm{atan}\left(y\right)$, etc.
    \item Use a non-breaking space when referencing something: Equation~1, Source~[1], Section~2.3. You can do this by using the tilde ``\verb|~|'', e.g. \verb|Equation~\ref{eq:xxx}|.
    \item Use \verb|\left(| and \verb|\right)| when setting parentheses in equations. This is correct: \verb|\left(\sum\right)|, this is wrong: \verb|(\sum)|.
    \item Units get a half non-breaking space: 1\,cm, that is \verb|1\,cm|. Units are also not set in italics.
    \item The English word ``exemplary'' is more commonly understood as ``very good and suitable to be copied by other people'' and not so much as ``exemplarisch'' in German.
    \item Text in figures should not be much smaller than the text in the captions.
    \item Whenever possible use figures in vector format.
    \item Treat equations as if they are part of the text, this means they get correct punctuation, example: This means
    \begin{equation}
        x = y,
    \end{equation}
    with $y$ being something, and
    \begin{equation}
        y = z.
    \end{equation}
    \item When indicating a multiplication, I prefer the \verb|\cdot| and not the \verb|*|, i.e. $x\cdot y$ and not $x*y$.
\end{itemize}



\chapter{Appendix - About this file}

\hint{The following is the German text from the original TU Darmstadt template document.}
